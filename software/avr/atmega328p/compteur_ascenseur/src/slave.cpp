...
void TWI_Loop( void )
{
	if ( ! TWI_Transceiver_Busy() ) {
		...
		if ( order == MASTER_CMD_RESET ) {
			communication.codeuseMoteurAvant.compteur(0);
			communication.codeuseMoteurArriere.compteur(0);
		}

		if ( order == MASTER_CMD_ALL ) {
			int32_t ascenseurAvant;
			int32_t ascenseurArriere;
			ascenseurAvant = communication.codeuseMoteurAvant.compteur();
			ascenseurArriere = communication.codeuseMoteurArriere.compteur();

			messageBuf[0] = (uint8_t) ascenseurAvant;
			messageBuf[1] = (uint8_t) (ascenseurAvant >> 8);
			messageBuf[2] = (uint8_t) (ascenseurAvant >> 16);
			messageBuf[3] = (uint8_t) (ascenseurAvant >> 24);

			messageBuf[4] = (uint8_t) ascenseurArriere;
			messageBuf[5] = (uint8_t) (ascenseurArriere >> 8);
			messageBuf[6] = (uint8_t) (ascenseurArriere >> 16);
			messageBuf[7] = (uint8_t) (ascenseurArriere >> 24);
		}
