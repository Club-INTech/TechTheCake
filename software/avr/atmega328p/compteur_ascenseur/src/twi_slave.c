...
void TWI_Loop( void )
{
    if ( ! TWI_Transceiver_Busy() ) {
        ...
        if ( order == MASTER_CMD_RESET ) {
            codeuseMoteurAvant.compteur(0);
            codeuseMoteurArriere.compteur(0);
        }

		if ( order == MASTER_CMD_ALL ) {
            int32_t ascenseurAvant = codeuseMoteurAvant.compteur();
            int32_t ascenseurArriere = codeuseMoteurArriere.compteur();
